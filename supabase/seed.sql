...
--
-- Isi: 1 modul (array-dasar), 5 bagian, 10 kartu, 10 soal, 40 opsi, 1 video
-- =========================================================
...
b as (
  ...
  from m, (values
    ('apa-itu-array', 'Apa Itu Array',
     'Array adalah kumpulan elemen dengan tipe yang sama yang disimpan berurutan di memori. Bayangkan deretan loker bernomor berurutan di sekolah: setiap loker berukuran sama dan posisinya berdampingan.', 1),
    ('memori-berurutan', 'Memori Berurutan',
     'Elemen array disimpan berdampingan di memori tanpa celah. Karena ukurannya seragam, alamat elemen ke-i bisa dihitung langsung dengan rumus: alamat(i) = alamat(0) + i x sizeof(Tipe).', 2),
    ('indeks', 'Indeks dan Pengaksesan',
     'Indeks dimulai dari 0, bukan 1. Mengapa? Karena indeks sebenarnya menyatakan jarak (offset) dari awal array. Elemen pertama berjarak 0 dari awal, sehingga berada di arr[0].', 3),
    ('batas-array', 'Batas Array',
     'C++ tidak memeriksa batas indeks saat runtime. Mengakses elemen di luar batas (seperti arr[5] pada array berukuran 5) menghasilkan undefined behavior: program mungkin crash, membaca nilai sampah, atau tampak berjalan normal tetapi merusak data lain.', 4),
    ('cpp-vs-python', 'C++ vs Python',
     'C++ memakai array statis dengan ukuran tetap dan tipe seragam. Python memakai list yang ukurannya bisa berubah dinamis dan elemennya berupa referensi ke objek bebas.', 5)
  ) as v(slug, judul, konten_md, urutan)
  returning id
),

-- Kartu flashcard (10 kartu)
k as (
...
from public.modul m, (values
  ('8WhUADLI4RQ', 'Belajar C++ [Dasar] - 42 - Pendahuluan Array', 'Konsep dasar array, deklarasi, pengindeksan elemen, serta representasi elemen array yang tersimpan berurutan di memori.', 1033, 1)
